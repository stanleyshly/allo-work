...
def main():
    ...
    v0:PynqBuffer = pynq.allocate(size, dtype=np.float32)
    v1:PynqBuffer = pynq.allocate(size, dtype=np.float32)

    v0[:] = np.random.rand(size).astype(np.float32)
    ...
    vvadd_0_hw.register_map.v0_1 = v0.physical_address
    vvadd_0_hw.register_map.v1_1 = v1.physical_address
